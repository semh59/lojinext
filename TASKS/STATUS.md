...
| **FAZ1** — Kod sınırları (17 kalem) | ✅ TAMAMLANDI (2026-07-23) — 17/17 kalem main'de, PR #1 merge edildi (`b761c7a`), main'de gerçek CI (`hard-gates`) ardışık 2 koşumda yeşil (`b761c7a`, `38dfceb`) | Dalga 1-16 main'de/branch'te yeşil (aşağıdaki tablo). Dalga 17 (platform-infra) 2026-07-22'de 10 commit'lik bir alt-dalgayla tamamlandı: cache/events/container/resilience/database/monitoring/security/context/logging/audit/background/middleware/websocket taşındı + `public.py` + kendi `public-surface-only-platform_infra` import-linter kontratı yazıldı + CI'nın blocking gate adımına eklendi (`.github/workflows/ci.yml`, önceden eksikti — bu turda düzeltildi). Ardından "V2 dışında kalan var mı" denetiminde bulunan 4 ek kalem de taşındı/silindi (`app/api/middleware/rate_limiter.py`→platform_infra, `app/api/v1/utils.py`→platform_infra, ölü `app/core/services/ai_service.py` silindi, `app/workers/tasks/{backup_tasks,error_digest}.py`→platform_infra/background). Her adım tam doğrulamayla (ruff/mypy/lint-imports/alembic check/pytest) ayrı commit+push edildi. **2026-07-23'te bulunup düzeltilen 6 gerçek CI engelleyicisi** (bkz. git log `c3c25ae`→`38dfceb`): bayat Celery import yolu, axios CVE, coverage'ın `v2/`'yi ölçmemesi, `conftest.py`'de eksik ORM model import'ları (28→42 tablo), ruff import-sort, bayat `frontend/openapi.json` snapshot'ı + 1 bozuk test mock hedefi. **KULLANICI KARARI (2026-07-23): "5 ardışık gün main'de yeşil" burn-in penceresi bilinçli olarak ATLANDI** ("5 gün bekleyemem, yeni branch'te FAZ2'ye başlarım") — main'de 2 gerçek ardışık yeşil koşum (takvim-günü değil, commit-bazlı) yeterli kabul edilip FAZ2'ye geçildi. |
| **FAZ2** — Veri sınırları | 🟡 BAŞLADI (2026-07-23, branch `faz2-veri-sinirlari`, `main`'in `38dfceb`'inden) | Henüz hiçbir FAZ2 görev dosyası (`faz2-schema-per-module-postgres.md`, `faz2-db-rol-izolasyonu-ve-read-model-grantlari.md`, `faz2-guvenlik-state-redis.md`) uygulanmadı — DURMA NOKTASI kuralı gereği her biri ayrı kullanıcı onayı bekliyor. |
