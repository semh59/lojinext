...
7. **"platform şeması" 6 tablosu tek blok değil — 3'ü admin_platform'a taşınabilir, 3'ü hiçbir v2 modülüne taşınamaz (2026-07-21, dalga 16 madde-3 ön-denetimi)**: `MEMORY/PROGRESS.md` §2.2'nin "platform (şema)" satırı (`sistem_konfig, konfig_gecmis, outbox_events, error_events, error_occurrences, idempotency_keys` — 6 tablo) tek bir 16. sahiplik gibi okunuyor ama gerçek grep + admin_platform'un KENDİ dalga-15 çıktısı (`v2/modules/admin_platform/application/error_events.py`'nin docstring'i: "error_events/error_occurrences/error_hourly_stats'ın YAZIM yolu app/infrastructure/monitoring/'de yaşıyor... hiçbir v2 modülüne ait değil") bunu ikiye ayırıyor:
   ...
   **Düzeltme**: madde 3 aşağıda güncellendi — gerçek dağıtım **40 tablo → 15 iş modülü + admin_platform (toplam 16 hedef)**, geri kalan **3 tablo (`outbox_events`, `error_events`, `error_occurrences`) `app/database/models.py`'de KALIR** (gerekçesiyle dokümante edilir, dosya bu yüzden BU dalgada tam silinemez) — gerçek sahipleri (`app/infrastructure/monitoring/`, `app/infrastructure/events/`) migrate olunca (muhtemelen `platform_infra`/dalga 17 veya ayrı bir infra-migration görevi) ele alınacak.
...
**3 tablo bu dalgada taşınmaz** (madde 0 düzeltme #7): `outbox_events`, `error_events`, `error_occurrences` — gerçek sahipleri `app/infrastructure/events/`ve `app/infrastructure/monitoring/`, henüz hiçbir v2 modülüne taşınmadı (audit_logger.py/event_bus.py ile aynı kategori). Bu 3 sınıf `app/database/models.py`'de KALIR, dosyanın başına gerekçe yorumu eklenir — `app/database/models.py` bu dalgada TAM silinmez (yalnız 43→3 sınıfa küçülür).
...
- [ ] models.py'nin 40 tablosu 15 iş modülü + admin_platam'a (16 hedef) dağıtıldı, her adımda `alembic check` boş-diff (madde 0 düzeltme #7 — geri kalan 3 tablo `outbox_events`/`error_events`/`error_occurrences` gerekçesiyle `app/database/models.py`'de kalıyor, bu BİLİNÇLİ bir eksik-taşıma, hata değil)
