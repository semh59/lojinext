...
- [x] models.py'nin 40 tablosu 15 iş modülü + admin_platform'a (16 hedef) dağıtıldı, her adımda `alembic check` boş-diff. Kalan 3 tablo (`outbox_events`/`error_events`/`error_occurrences`) de taşındı (2026-07-21, kullanıcı geri bildirimiyle) — `v2/modules/shared_kernel/infrastructure/{outbox,error_monitoring_models}.py`. `app/database/models.py` artık 43 tablo içermiyor.
