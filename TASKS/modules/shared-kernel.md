...
> **DURMA NOKTASI:** Kullanıcı onayı olmadan uygulanmaz. **1. Adım:** `v2/shared_kernel/CLAUDE.md`'yi Read ile oku (yoksa oluştur).
...
**Giriş kriteri:** admin-platform dalgası (15) tamamlandı — tüm iş modülleri artık `v2/modules/` altında. **Çıkış kriteri:** `v2/shared_kernel/` dizini yalnız GERÇEKTEN ≥2 modül tarafından kullanılan kod içeriyor; cross-module her yeni giriş CODEOWNERS onaylı.
...
5. **public.py/events.py kararı hiç yoktu**: diğer 15 modülün HEPSİNDE zorunluydu, burada hiç bahsi geçmiyordu. shared_kernel iş modülü olmadığı için `public-surface-only-X` tipi bir import-linter kısıtı anlamsız (amaç zaten HERKESİN serbestçe import edebilmesi) — ama "hangi sembollerin kasıtlı dış-yüzey olduğu" belirsiz kalırsa gelecekte rastgele iç-detay importları (`v2.shared_kernel.infrastructure.base.Base` gibi dağınık importlar) çoğalabilir. **Düzeltme**: madde 7 (yeni) — minimal bir `public.py` eklenmesi kararlaştırıldı, zorunlu import-linter kontratı olmadan, yalnız disiplin için.

---

## 1. Mevcut envanter (22 dosya, ~5.223 LOC — gerçek `wc -l` ile doğrulandı)
```
app/__init__.py                          → v2/shared_kernel/__init__.py (boş)
app/core/__init__.py                     → v2/shared_kernel/__init__.py ile birleşir
...
app/schemas/base.py                      → v2/shared_kernel/schemas/base.py (gerçek kullanım taranacak)
app/schemas/api_responses.py             → BÖLÜNÜR (madde 4) — 846 satır
app/schemas/validators.py                → v2/shared_kernel/schemas/validators.py (gerçek kullanım taranacak)
...
app/core/errors.py                       → v2/shared_kernel'de KALIR (main.py bootstrap seviyesi, tek "modül" değil)
app/core/exceptions.py                   → v2/shared_kernel'de KALIR (57 gerçek çağıran, çoklu modül)
...
app/core/utils/__init__.py               → v2/shared_kernel/utils/__init__.py
app/core/utils/clock.py                  → gerçek kullanım taranacak, v2/shared_kernel'de KALIR (varsayım)
app/core/utils/type_helpers.py           → gerçek kullanım taranacak, v2/shared_kernel'de KALIR (varsayım)
app/database/__init__.py                 → v2/shared_kernel/__init__.py ile birleşir
app/database/repositories/__init__.py    → TAMAMEN SİLİNİR (madde 5) — re-export hunisi
app/database/base_repository.py          → v2/shared_kernel/infrastructure/base_repository.py (çoklu modül kullanıyor — BaseRepository generic)
app/database/unit_of_work.py             → v2/shared_kernel/infrastructure/unit_of_work.py (gerçek UoW — TEK gerçek versiyon, app/core/unit_of_work.py'nin shim'i değil bu)
```
...
`app/database/models.py` (1.988 satır, 43 ORM tablosu, 45 `relationship()`) BU DALGADA modül-başına dağıtılır: her tablo `v2/modules/<sahip>/infrastructure/models.py`'ye taşınır (sahiplik MEMORY/PROGRESS.md §2.2 tablosundan — 15 iş modülü + platform şeması). Ortak `Base` sınıfı BURADA (`v2/shared_kernel/infrastructure/base.py`) kalır, her modül ondan miras alır.
...
| `BaseEntity` | v2/shared_kernel | gerçekten paylaşılan taban Pydantic sınıfı — TEK KALAN shared_kernel üyesi bu dosyadan |
...
## 6. Kabul kriterleri
- [ ] Tüm yol referansları `v2/shared_kernel/` ve `v2/modules/<isim>/` (madde 0 düzeltme #1)
...
- [ ] models.py 15+1 hedefe modül-modül dağıtıldı, her adımda `alembic check` boş-diff
...
- [ ] `v2/shared_kernel/public.py` oluşturuldu — kasıtlı dış-yüzey sembolleri (madde 7)
