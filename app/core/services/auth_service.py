    ...
    async def refresh_session(self, refresh_token: str) -> Tuple[str, str]:
        ...
        async with self.uow:
            ...
            for session in sessions:
                if jwt_handler.verify_token_hash(
            ...
            target_session.refresh_token_hash = jwt_handler.hash_token(
                new_refresh_token
            )
    ...
    async def reset_password(self, token: str, new_password: str) -> bool:
        ...
        async with self.uow:
            ...
            if not user:
                return False
            if (
                user.sifre_sifir_son
                and datetime.now(timezone.utc) > user.sifre_sifir_son
            ):
