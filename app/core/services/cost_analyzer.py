...
class CostAnalyzer:
    """
    Production cost analytics service.

    This service is restricted to source-backed calculations only. It must not
    fabricate ROI or savings numbers when the underlying fuel and trip data are
    missing.
    """
    ...
    async def calculate_period_cost(
        self, start_date: date, end_date: date, arac_id: Optional[int] = None
    ) -> CostBreakdown:
    ...
    async def get_vehicle_cost_comparison(self, months: int = 3) -> List[Dict]:
        ...
        async def calculate_for_vehicle(vehicle):
            ...
            async with _sem:
                try:
                    breakdown = await self.calculate_period_cost(
                        start_date, today, arac_id
                    )
                    return {
                        "arac_id": arac_id,
    ...
    async def calculate_savings_potential(
        self, target_consumption: float = 30.0
    ) -> Dict:
        ...
        if current.total_distance <= 0 or current.fuel_liters <= 0:
            return {
                "error": "Savings analysis requires source-backed trip and fuel data."
            }

        current_cost = float(current.fuel_cost)
        current_consumption = round(
            current.fuel_liters / (current.total_distance / 100), 2
        )
        target_liters = (current.total_distance / 100) * target_consumption
        target_cost = float(
            round(target_liters * float(current.avg_price_per_liter), 2)
        )
    ...
    async def calculate_roi(
        ...
        savings_result = await self.calculate_savings_potential(target_consumption)
