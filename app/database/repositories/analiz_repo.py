...
from app.database.models import Anomaly, FuelInvestigation, Sefer, YakitFormul
...
class AnalizRepository(BaseRepository[Sefer]):
    ...
    # =========================================================================
    # FUEL INVESTIGATIONS (Feature B.2 — Tier F madde 35: raw SQL/ORM query
    # relocation from app/api/v1/endpoints/investigations.py into the
    # repository layer)
    # =========================================================================
    ...
    async def get_investigation_detail(self, inv_id: int) -> Optional[Dict[str, Any]]:
        ...
        sql = self._INVESTIGATION_JOIN_SQL + " AND fi.id = :id LIMIT 1"
        row = (
    ...
    async def list_investigations(
        ...
        sql = self._INVESTIGATION_JOIN_SQL
