...
_SETTINGS_PATH = "v2.modules.notification.api.push_routes.settings"
...
async def test_vapid_key_push_disabled(async_client, admin_auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        "v2.modules.notification.api.push_routes.settings.VAPID_PUBLIC_KEY", ""
    )
    monkeypatch.setattr(
        "v2.modules.notification.api.push_routes.settings.PUSH_NOTIFICATION_ENABLED",
        False,
    )
...
async def test_vapid_key_push_enabled(async_client, admin_auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        "v2.modules.notification.api.push_routes.settings.VAPID_PRIVATE_KEY",
        "fake-priv-key",
    )
    monkeypatch.setattr(
        "v2.modules.notification.api.push_routes.settings.PUSH_NOTIFICATION_ENABLED",
...
async def test_test_push_calls_send_push(async_client, admin_auth_headers, monkeypatch):
    ...
    with patch(
        "v2.modules.notification.api.push_routes.send_push_to_user",
...
def test_vapid_public_key_response_schema():
    from v2.modules.notification.schemas import VapidPublicKeyResponse
