...
Targets missing lines in app/api/v1/endpoints/trailers.py (~30% → ≥75%).
...
@contextmanager
def _override_dorse_service(mock_svc):
    ...
    app.dependency_overrides[get_dorse_service] = _fake
    try:
        yield
    finally:
        app.dependency_overrides.pop(get_dorse_service, None)
...
async def test_list_trailers_happy_path(async_client, admin_auth_headers):
    ...
    mock_svc = AsyncMock()
    mock_svc.get_all_paged = AsyncMock(return_value=[_make_dorse_response()])
...
async def test_list_trailers_with_search(async_client, admin_auth_headers):
    """Search param is forwarded to service."""
    mock_svc = AsyncMock()
    mock_svc.get_all_paged = AsyncMock(return_value=[])
...
async def test_list_trailers_service_exception(async_client, admin_auth_headers):
    ...
    mock_svc = AsyncMock()
    mock_svc.get_all_paged = AsyncMock(side_effect=Exception("DB failure"))
...
async def test_create_trailer_value_error(async_client, admin_auth_headers):
    ...
    mock_svc = AsyncMock()
    mock_svc.create = AsyncMock(side_effect=ValueError("Plaka zaten mevcut"))
...
async def test_update_trailer_not_found(async_client, admin_auth_headers):
    """Service returns False → 404."""
    mock_svc = AsyncMock()
    mock_svc.update = AsyncMock(return_value=False)
...
async def test_update_trailer_happy_path(async_client, admin_auth_headers):
    ...
    mock_svc = AsyncMock()
    mock_svc.update = AsyncMock(return_value=True)
...
async def test_update_trailer_value_error(async_client, admin_auth_headers):
    """ValueError from service → 400."""
    mock_svc = AsyncMock()
    mock_svc.update = AsyncMock(side_effect=ValueError("Geçersiz plaka"))
...
async def test_delete_trailer_service_exception(async_client, admin_auth_headers):
    ...
    mock_svc = AsyncMock()
    mock_svc.delete = AsyncMock(side_effect=RuntimeError("DB error"))
...
async def test_export_trailers_happy_path(async_client, admin_auth_headers):
    ...
    mock_svc = AsyncMock()
    mock_svc.export_all_trailers = AsyncMock(return_value=b"PK\x03\x04fakexlsx")
...
async def test_get_template_happy_path(async_client, admin_auth_headers):
    ...
    mock_svc = AsyncMock()
    mock_svc.get_template = AsyncMock(return_value=b"PK\x03\x04template")
...
async def test_import_trailers_exception(async_client, admin_auth_headers):
    """Exception from service → 400."""
    mock_svc = AsyncMock()
    mock_svc.import_trailers = AsyncMock(side_effect=Exception("Parse error"))
