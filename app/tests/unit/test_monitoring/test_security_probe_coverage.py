...
def test_brute_force_lru_evicts_when_full():
    ...
    from v2.modules.platform_infra.monitoring import security_probe as sp
...
def test_rbac_lru_evicts_when_full():
    ...
    try:
        ...
        tracker = RBACViolationTracker()
        with patch("v2.modules.platform_infra.monitoring.emit"):
            tracker.record(1, "/a")
            tracker.record(2, "/b")
            tracker.record(3, "/c")
...
def test_rbac_endpoints_sample_included_in_event():
    ...
    with patch(
        "v2.modules.platform_infra.monitoring.emit", side_effect=lambda ev: captured.append(ev)
    ):
        for i in range(20):
