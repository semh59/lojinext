...
function'ları (``evaluate_driver``, ``get_all_evaluations``,
``calculate_verimlilik_puan`` vb.) doğrudan çağırır; repo mock'ları bir
``mock_uow`` fixture'ı üzerinden ``uow=`` kwarg'ıyla geçirilir.
"""
...
from v2.modules.driver.domain.evaluation import (
    ...
    SoforDegerlendirme,
    TrendEnum,
    calculate_deneyim_puan,
    calculate_trend_puan,
    calculate_tutarlilik_puan,
    calculate_verimlilik_puan,
    evaluate_driver,
...
def test_verimlilik_puan_equal_to_average_is_50():
    assert calculate_verimlilik_puan(32.0, 32.0) == 50.0
...
def test_tutarlilik_puan_zero_std_is_100():
    assert calculate_tutarlilik_puan(0.0) == 100.0


def test_tutarlilik_puan_std_2_is_80():
    assert calculate_tutarlilik_puan(2.0) == 80.0


def test_tutarlilik_puan_std_5_is_50():
    assert calculate_tutarlilik_puan(5.0) == 50.0


def test_tutarlilik_puan_std_10_floors_at_0():
    assert calculate_tutarlilik_puan(10.0) == 0.0


def test_tutarlilik_puan_high_std_floors_at_0():
    assert calculate_tutarlilik_puan(20.0) == 0.0
...
def test_deneyim_puan_zero_km_zero_sefer():
    result = calculate_deneyim_puan(0, 0)
    # log10(1) = 0 for both
    assert result == 0.0
...
def test_deneyim_puan_large_km_and_sefer_caps():
    ...
    result = calculate_deneyim_puan(1_000_000, 1000)
    assert result == pytest.approx(expected, abs=0.1)


def test_deneyim_puan_moderate_values():
    result = calculate_deneyim_puan(100_000, 100)
    assert 0 < result <= 100
...
# ---------------------------------------------------------------------------
# evaluate_driver with pre_metrics
# ---------------------------------------------------------------------------


async def test_evaluate_driver_with_pre_metrics(mock_uow):
...
async def test_evaluate_driver_stable_trend_when_no_recent(mock_uow):
    mock_uow.analiz_repo.get_filo_ortalama_tuketim = AsyncMock(return_value=32.0)
