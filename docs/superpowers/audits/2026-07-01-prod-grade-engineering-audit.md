...
**Mega-run doğrulama notu:** Tam paket regresyonda (backend 6209/6219 unit+api, integration 242/265, frontend 1188/1191) Tier A dışı dosyalarda dağınık hatalar çıktı. Örnek doğrulama: `test_idempotency_key.py`'nin 6 hatası TÜM Tier A değişiklikleri stash'lenip AYNI şekilde reprodüklendi → önce "pre-existing, Tier A ile ilgisiz" işaretlendi. **Sonra kök neden bulundu (fork-agent, 2026-07-02): kod bug'ı değil.** Dosya izole 3 kez art arda çalıştırılıp 3/3 kez 6/6 PASS bulundu — hatalar gözlemlendiğinde aynı host'ta 2 ayrı ağır Docker pytest mega-run'ı ZATEN arka planda çalışıyordu; CPU/DB-bağlantı çekişmesi `sefer_write_service.py`'nin 2.5s prediction-timeout'unu geçici tetikleyip kaskad hatalara yol açtı. Kod tabanında düzeltilecek bir şey yok. Her Tier A maddesi kendi izole+bitişik-regresyon turunda ayrı ayrı 100% doğrulandı.

**Ek not — `UnitOfWork.rollback()` (madde 2) için ikinci derin tur, konklüzif:** kullanıcı talimatıyla ayrı bir tasarım oturumu açıldı. SAVEPOINT-tabanlı bir alternatif, SQLAlchemy 2.0'ın gerçek API'si (`AsyncSession.begin_nested()`/`AsyncSessionTransaction`, container içinde `inspect` ile doğrulandı) kullanılarak gerçek Postgres test DB'sine karşı ampirik test edildi: manuel savepoint rollback güvenli, ama `asyncio.wait_for` ile GERÇEK bir sorgu ortasında iptal edilince (`pg_sleep(5)`, timeout=0.3s) savepoint rollback'in kendisi "başarılı" dönse bile sonraki outer işlem yine `PendingRollbackError` ile patlıyor — mid-query cancellation asyncpg bağlantısının kendisini (wire-protokol seviyesinde) geçersiz kılıyor, ORM/SAVEPOINT seviyesinde atlatılamıyor. **Sonuç: mevcut "nested UoW'da herhangi bir exception → tam session rollback" davranışı bug değil, gerekli bir savunma mekanizması — genel mekanizma değiştirilmemeli.** Doğru düzeltme deseni madde 19'da (idempotency) zaten uygulanmıştı: rollback riskine maruz kalmaması gereken durumları tamamen bağımsız bir session'a taşımak. Fork'un bulduğu ikinci risk noktası (`sefer_write_service.py:1442`, `bulk_add_sefer`/Excel toplu import — aynı `asyncio.wait_for`+nested-UoW-prediction deseni) silent veri kaybı riski taşımıyor (henüz DB'ye yazılmamış satırlar) ama tüm batch'in nadiren 500 ile patlama riski var — ayrı, küçük bir karar bekliyor (kabul edilebilir mi, yoksa idempotency'deki gibi bağımsız session'a mı taşınmalı).

---

### 28. Tier B — konsolide plan madde 7-9, 2026-07-02

**28a (madde 7) — Plaka validasyonu 3 farklı yerde 3 farklı regex.** **KAPANDI** — canlı `POST /vehicles/` akışı `schemas/arac.py`'nin "Permissive" pattern'ini kullanıyor (`entities/models.py::validate_plaka_str`'nin STRICT versiyonu gerçekte hiç çağrılmıyor, doğrulandı — dokunulmadı, davranış etkisi yok). Gerçek çelişki `schemas/arac.py` (azami 5 harf + Türkçe karakter) ile `import_service.py::_validate_plaka` (azami 3 harf, Türkçe karakter yok) arasındaydı. Fix: `app/schemas/validators.py`'a tek kaynak `PLAKA_PATTERN` eklendi (gevşek yöne — canlı API'yi kısıtlamadan Excel'i ona hizalayarak), 3 kullanım noktası buna yönlendirildi. Yeni test kırmızı-yeşil doğrulandı. 157+71 test regresyon PASS.

**28b (madde 8) — `sefer_write_service.py`'de 2.5s tahmin timeout'u 3 yerde hardcoded.** **KAPANDI** — modül-seviyesi `_PREDICTION_TIMEOUT_SECONDS = 2.5` sabiti eklendi, 3 kullanım noktası + 2 log mesajı buna yönlendirildi. Yeni regresyon-guard testi (dosya kaynağını tarayıp başka `timeout=<literal>` kalmadığını doğruluyor) + mevcut timeout-davranış testi yeşil doğrulandı. 43 test regresyon PASS.
...
**28e (madde 11) — ARIMA fit'i geniş `except Exception` ile moving-average'a düşüp `success:True` sahte-başarı dönüyor.** **KAPANDI** — `ARIMATimeSeriesPredictor` (`time_series_predictor.py`) canlıda ZERO çağrıya sahip olduğu grep ile doğrulandı (`app/workers`/`app/api`/`container.py`/`app/services` boş); gerçek üretim yolu `advanced_lstm.py::AdvancedTSEngine`, orada `success`/`method` zaten doğru yönetiliyor. Fix, düşük riskli dead-code sınıfına uygulandı: `predict()`'e `degraded: bool` alanı eklendi (`success` semantiği DEĞİŞMEDİ, mevcut testler kırılmadı) — moving_average fallback `degraded=True`, gerçek ARIMA başarısı `degraded=False`. Yeni test kırmızı-yeşil doğrulandı. 56+ test regresyon PASS (1 pre-existing, ortam-bağımlı, ilgisiz fail hariç — `git stash` ile fix'siz aynı hata reprodüklendi).

**28f (madde 12) — `Arac` yaş hesabı iki ayrı kaynaktan.** **KAPANDI — audit'in tahmininden DAHA CİDDİ çıktı.** `sefer_fuel_estimator.py::_derive_arac_yasi` `arac.uretim_tarihi` okuyordu ama `araclar` tablosunda böyle bir kolon HİÇ YOK (`app/database/models.py::Arac` sadece `yil` taşıyor) — canlıda `getattr(arac, "uretim_tarihi", None)` HER ZAMAN `None`, `_derive_arac_yasi` HER araç için sessizce sabit `5` dönüyordu (age_factor hiç değişmiyordu, feature fiilen devre dışıydı). Mevcut 5 test bunu `MagicMock().uretim_tarihi = date(...)` ile maskeliyordu (gerçek ORM nesnesinde asla var olmayan bir alan). Fix: `_derive_arac_yasi` artık `arac.yil`'den hesaplıyor — `entities/models.py::Arac.yas` (`ensemble_service.py`'nin kullandığı) ile AYNI tek kaynak. 2 yeni test eklendi (biri iki hesaplamanın birebir aynı sonucu ürettiğini doğruluyor). 33 test regresyon PASS.

**28g (madde 13) — `import_service.py` dış catch tüm hataları "sistem hatası"na çeviriyor.** **KAPANDI.** Doğrulama: `main.py`'nin Sentry `LoggingIntegration`'ı `event_level=None` — `logger.error()` TEK BAŞINA Sentry event'i OLUŞTURMUYOR, sadece `capture_exception()` veya projenin `aemit(ErrorEvent(...))` (AlarmRouter) mekanizması gerçek alarm tetikliyor. 5 fonksiyonun (`process_sefer_import`/`process_yakit_import`/`process_vehicle_import`/`process_driver_import`/`import_routes`) dış catch'i exception'ı TAMAMEN yutuyordu — FastAPI'nin global handler'ına asla ulaşmıyordu, ne Sentry ne `aemit` tetikleniyordu. Fix: `(count, errors)` dönüş sözleşmesi KORUNARAK `_report_infra_failure()` helper'ı eklendi — `logger.error(exc_info=True)` + `aemit(ErrorEvent(layer=SERVICE, severity=CRITICAL))`. 2 yeni test (`aemit` mock'lanıp doğru layer/severity/category doğrulanıyor) kırmızı-yeşil doğrulandı. 162 test regresyon PASS.

**28h (madde 14) — `route_repo.py` koordinat sorgusu composite/mekânsal index'siz.** **ARAŞTIRILDI, AUDIT İDDİASI YANLIŞ ÇIKTI, KÜÇÜK GERÇEK TEMİZLİK YAPILDI.** EXPLAIN ANALYZE ile (gerçek Postgres, 100k satır, throwaway probe tablosu) ampirik olarak yanlışlandı: `uq_route_coords` UniqueConstraint zaten composite bir btree index üretiyor VE query planner `get_by_coords`'un 4-kolonlu AND-range sorgusu için bunu kullanıyor (Index Scan, <1ms — Tier A'daki "VEHICLE_AGE_DEGRADATION_RATE ölü kod" false-alarm'ıyla aynı desen). Gerçek bulgu: 4 kolonun HER BİRİ AYRICA `index=True` ile gereksiz tekil index'e sahipti (hiçbir kod tek kolon sorgulamıyor) — sadece INSERT/UPDATE maliyeti. Fix: 4 tekil index kaldırıldı, migration `0039_route_idx_cleanup` — throwaway scratch DB'de upgrade/downgrade/re-upgrade/`alembic check` doğrulandı. Yeni regresyon-guard testi (`pg_indexes` katalog sorgusu) kırmızı-yeşil doğrulandı. 31 test regresyon PASS.

**28i (madde 15) — Excel import satır-sayısı üst sınırı yok.** **KAPANDI.** TÜM import yolları tarandı: `excel_parser.py`'deki 6 `parse_*_sync` fonksiyonu YETMEZ, çünkü `import_service.py`'nin KENDİ 2 doğrudan `pd.read_excel` çağrısı (`parse_and_preview`, `_parse_import_file`) `ExcelService`'i hiç kullanmıyor, ayrı guard gerektiriyordu. Fix: ortak `MAX_EXCEL_ROWS = 20_000` sabiti, 6 parser'a `_enforce_row_limit()` (aşılırsa `ExcelExportError`, `DomainError` alt sınıfı) + `import_service.py`'nin 2 doğrudan okuma noktasına ayrı guard (`parse_and_preview` → HTTP 413, `_parse_import_file` → `ExcelExportError`). 6 yeni test (`MAX_EXCEL_ROWS` monkeypatch'lenerek hızlı/deterministik) kırmızı-yeşil doğrulandı. 333 test regresyon PASS.

**Regresyon (28a-i):** `ruff`+`mypy` temiz (baseline 7, aynı, değişen dosyalarda yok). **Tier B tamamlandı: 9/9 madde (7-15).**
