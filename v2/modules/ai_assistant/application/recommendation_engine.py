...
Ölü kod (taşındı, silinmedi): `RecommendationEngine`/`get_recommendation_engine`
hiçbir prod endpoint/servisten çağrılmıyor — grep ile doğrulandı, yalnız
`test_recommendation_engine_coverage.py`/`test_recommendation_more.py`
tarafından egzersiz ediliyor. `_cache`/`_cache_time`/`_lock` gerçek mutable
state taşıdığı için B.1 sınıf istisnası (RAGEngine/RecommendationEngine ile
aynı gerekçe) — InsightEngine (dalga 11) gibi silinmedi, kullanıcı kararı
bekliyor.
"""
