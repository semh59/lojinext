"""
TIR Yakıt Takip - Yakıt Tahmin (basit regresyon)
ML tabanlı yakıt tahmini iş mantığı — prediction_ml'in EnsembleFuelPredictor'ından
FARKLI, modül-içi hafif bir regresyon pipeline'ı.

Hiçbir API endpoint'i bu fonksiyonları çağırmıyor (doğrulandı, dalga 4) —
``app/services/prediction_service.py``'deki ``YakitTahminService()`` referansı
hiç kullanılmayan bir constructor attribute'uydu (dead code, taşınmadı).
"""
...
_ZORLUK_MAP = {"Normal": 0, "Orta": 1, "Zor": 2}
    ...
    for s in seferler:
        ...
        X_list.append(
            [
                ...
                float(_ZORLUK_MAP.get(s.get("zorluk", "Normal"), 0)),
...
async def predict(
    ...
    # Her çağrıda yeni model nesnesi — singleton bir modeli overwrite etmez
    ...
    # Zorluk dönüşümü
    z_val = float(_ZORLUK_MAP.get(zorluk, 0))
