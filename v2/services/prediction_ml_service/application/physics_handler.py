...
class PhysicsRecalculationHandler:
    ...
    async def on_sefer_updated(self, event: Event):
        ...
        sefer = await cross_module_client.get_sefer(sefer_id)
        if not sefer:
            return

        arac = await cross_module_client.get_vehicle(sefer.get("arac_id"))
        if not arac:
            return
        ...
        if sefer.get("dorse_id"):
            dorse = await cross_module_client.get_trailer(sefer.get("dorse_id"))
        ...
        specs = VehicleSpecs(
            ...
            trailer_empty_weight_kg=dorse.get("bos_agirlik_kg") if dorse else 6000.0,
        ...
        # 7. Update Sefer
        await cross_module_client.update_tahmini_tuketim(
            sefer_id, prediction.total_liters
        )
